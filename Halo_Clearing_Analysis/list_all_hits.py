#!/usr/bin/env python3
"""
PETase halo & clearing hit finder

Outputs one Excel file with all z-scores and hit flags, but
_only_ for strains that have at least one hit:

    • 24 h halo   : z_24h >  2
    • 42 h halo   : z_42h >  2
    • 48 h clearing : z_48h < -2
"""
...
OUTPUT_XLSX  = Path("zScore_hits_summary.xlsx")

Z_HIGH = 2.0   # z > 2 hit for halo plates
Z_LOW  = -3.0  # z < −2 hit for clearing plate
# ---------------------------------------------------------------- helpers
...
summary = summary[hit_any].sort_values("strain").reset_index(drop=True)

# ---------- write Excel ---------------------------------------------------
with pd.ExcelWriter(OUTPUT_XLSX, engine="openpyxl") as wrt:
    summary.to_excel(wrt, sheet_name="hits", index=False)

print(f"✓ wrote {OUTPUT_XLSX}  ({len(summary)} strains with ≥1 hit)")
